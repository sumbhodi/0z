...
` + EPI,
  ...
  // 3 Sep 2026 — the shim also RELAYS THE FINGER: a touch inside the game (start · move · end, one finger) is posted to the
  //    parent as cabinet-gesture, and the desk's gesture door (topbar.js) reads it like its own. That is how a swipe inside
  //    the game fills the glass with the game, and a swipe down brings the desk back — an iframe's touches never bubble out.
  ...
  const PAUSE_SHIM = "(function(){var P=false;var rAF=window.requestAnimationFrame.bind(window);window.requestAnimationFrame=function(cb){return rAF(function tick(t){if(P){rAF(tick);return}cb(t)})};var sI=window.setInterval.bind(window);window.setInterval=function(fn,ms){var a=[].slice.call(arguments,2);return sI(function(){if(!P)(typeof fn==='function'?fn:new Function(fn)).apply(null,a)},ms)};var sT=window.setTimeout.bind(window);window.setTimeout=function(fn,ms){var a=[].slice.call(arguments,2);return sT(function tick(){if(P){sT(tick,120);return}(typeof fn==='function'?fn:new Function(fn)).apply(null,a)},ms)};window.addEventListener('message',function(e){var d=e.data||{};if(d.oz!=='cabinet-ctl')return;if(d.cmd==='pause')P=true;if(d.cmd==='resume')P=false});var gy=null;function relay(t,y){try{parent.postMessage({oz:'cabinet-gesture',type:t,y:y},'*')}catch(_){}}window.addEventListener('touchstart',function(e){if(e.touches&&e.touches.length===1){gy=e.touches[0].clientY;relay('start',gy)}else{gy=null}},{passive:true,capture:true});window.addEventListener('touchmove',function(e){if(gy!=null&&e.touches&&e.touches[0])relay('move',e.touches[0].clientY)},{passive:true,capture:true});window.addEventListener('touchend',function(){gy=null;relay('end',0)},{passive:true,capture:true});['keydown','keyup','pointerdown','mousedown','touchstart'].forEach(function(ev){window.addEventListener(ev,function(e){if(P)e.stopImmediatePropagation()},true)})})();"
